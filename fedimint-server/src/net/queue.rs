...
/// without an await point in between. 4. Once an acknowledgement is received
/// for a message, `ack` is called with its message id. All    messages with a
/// lower or equal id will be removed from the buffer. 5. If a reconnect happens
/// all messages that have not been acknowledged have to be resent. In
